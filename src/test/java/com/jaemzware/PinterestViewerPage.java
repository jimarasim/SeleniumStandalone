...
public class PinterestViewerPage extends BasePageObject {
    ...
    public PinterestHomePage closeViewer(){
        PinterestHomePage homePage = new PinterestHomePage(driver);

        closeButton.click();

        (new WebDriverWait(driver,10)).until(ExpectedConditions.elementToBeClickable(homePage.getSearchTextbox()));
